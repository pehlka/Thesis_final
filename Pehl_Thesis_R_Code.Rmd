...
lab_data2_avg_v_both <- read.csv("lab_data2_avg_v_both.csv")
  #lab racetrack data with average velocity
lab_data2_avg_v_both_omit <- lab_data2_avg_v_both[!is.na(lab_data2_avg_v_both$bipedal_percent), ]
  #lab racetrack data with average velocity - omitted non-bipedal rows
...
```{r}
#Avgerage Velocity ~ Substrate 

lme.both.avgv.sub <- lme(both_avg_v ~ substrate_type*biped_combined, random = ~1|ID, data = lab_data2_avg_v_both)
anova(lme.both.avgv.sub, type="marginal")  
shapiro.test(residuals(lme.both.avgv.sub))  
...
```{r}
#Bipedalism's effect on average velocity 

peakv.both.biped <- lme(both_avg_v ~ log10(bipedal_percent+1), random = ~1|ID, data = lab_data2_avg_v_both_omit)
anova(peakv.both.biped)  
shapiro.test(residuals(peakv.both.biped))  
performance::r2(peakv.both.biped) 
...
fieldspeed_jan$substrate <- factor(fieldspeed_jan$Substrate.Type, levels = c('DEBRIS', 'MIXED', 'SAND'))

ggplot(fieldspeed_jan, aes(x = substrate, y = Speed, fill = Substrate.Type)) + 
  geom_boxplot() +
